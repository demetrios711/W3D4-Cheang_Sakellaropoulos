...
def harrison_ford
  ...
  Movie.joins(:actors).select(:id, :title).where(actors: {name: "Harrison Ford"}).joins(:castings).where.not(castings: {ord:  1}).distinct
...
def biggest_cast
  ...
  Movie.joins(:castings).select(:id, :title).group('movies.id').order('COUNT(castings.actor_id) DESC').limit(3)
